...
jQuery(function($) {
	...
	*/
	...
	function getLastpost_meta(timestamp)
	{
		jQuery.ajax({
			type: "POST",
			url: wpUrl +'?page=last_post_meta',
			mode: "abort",
			//dataType: "json",
			//data: data,
			error: function(){
	...
	*/
	function newPost(trigger) {
		...
		$.ajax({
			...
			success: function(result) {
				...
				if(result!=0 && $('#attachmentfile').val())  //vaj changes start
				{
					getMetaTagChange(result);
				} //vaj changes end
				var notification_usersid = $('#notification_usersid').val();
				if(notification_usersid)
				{
					sendNotification(notification_usersid);	
					$('#notification_usersid').val('');
				}
	...
	function sendNotification(userIDs) //vaj changes start (whole functon)
	{
		jQuery.ajax({
			type: "POST",
			url: wpUrl+"?page=notify&uids="+userIDs,
			mode: "abort",
			error: function(){
			alert('Error loading information');
		},
	...
	function getMetaTagChange(postid) //vaj changes start (whole functon)
	{
		...
		var data = {
				'action'	: 'add-meta',
				'post_id'	: postid,
				'key'		: attachmentfile, //attachmentfile	
			};
		jQuery.ajax({
			type: "POST",
			url: wpUrl+"?page=ajax&post_id="+postid+"&afile="+attachmentfile,
			mode: "abort",
			//dataType: "json",
			data: data,
			error: function(){
